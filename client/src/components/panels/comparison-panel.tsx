...
export function ComparisonPanel({
  ...
  const SideEditor = ({
    ...
    const ph = getPlaceholders(global.theme, side);
    ...
    const removeItem = (idx: number) => update({ items: data.items.filter((_, i) => i !== idx) });
  ...
  return (
    <div className="space-y-3" data-testid="comparison-panel">
      ...
      {/* Per-side editors */}
      <SideEditor side="left" data={left} update={updateLeft} />
      <SideEditor side="right" data={right} update={updateRight} />
